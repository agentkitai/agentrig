...
- TUI composition: typing `/` shows a filtered list of loaded skills and commands;
  Up/Down chooses a suggestion and Tab fills it without execution. Enter runs the
  typed command; entering arguments closes the list. Outside the list, Up/Down
  recalls prompts. Distinguishable Shift-Enter or Enter after an odd trailing `\`
...
- **Memory:** `--memory <dir>` selects the wiki and enables its read/search tools. Automatic index injection is opt-in with `"memoryIndexInjection": true` in config. Session-end ingest defaults on (`"ingestOnEnd": false` opts out in config), subject to the [sandbox exception](docs/DEFAULTS.md). `--dream-on-end` runs a due dream in report-only mode; `--dream-every-sessions <n>` and `--dream-every-hours <n>` set its cadence, while `--dream-structural-only` skips the model-backed pass.
