...
import { SessionStore, SpendLedger, createAgent, meterProvider, RulePolicy } from "@agentkitai/agentrig-core";
...
it("actual controller /cost renders the completed current run and performs no new model call", async () => {
  ...
  const raw = { id: "fixture", model: "fixture", capabilities: { tools: true, parallelTools: false, caching: false, contextWindow: 10 },
    async *stream() { calls++; yield { type: "usage" as const, usage: { input: 10, output: 10 } }; yield { type: "stop" as const, reason: "end_turn" as const }; } };
  const provider = meterProvider(raw, ledger, { segment: "auxiliary", pricing: { inputUsdPerMTok: 1, outputUsdPerMTok: 1 } });
  const agent = createAgent({ provider, store: new SessionStore({ root: f.logs }), tools: [], permissions: new RulePolicy([]),
