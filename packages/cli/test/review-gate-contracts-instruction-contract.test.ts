...
const checks: Array<[string, string, string, string[]]> = [
 ["M-lander-fixer-receipt",skill("land"),"## 1.",["`gh pr view NN --json body` read-back receipt BEFORE", "Reject a missing read-back receipt", "Pre-dispatch read-back: Repair round: N/3; blockers <IDs>; OLD <SHA>; verified <ISO ts>", "in the GitHub PR body BEFORE dispatch", "match that line against the round, OLD and assigned blockers", "persisted fixer task", "counter repaired by the fixer afterwards cannot retroactively satisfy it", "matched to an immutable session-store `subagent.spawn` event", "same exact task text and child session ID", "does not waive receipt-before-dispatch ordering", "durable pre-edit comparison with comment ID and head", "If neither the durable fixer pre-push handoff nor that matched conductor-comment-plus-spawn provenance exists"]],
