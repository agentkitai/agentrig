...
it.each(["dogfood", "topic", "review", "arbiter"])("%s pins operative declared-checks policy", async (skill) => {
  const text = instructionSource(`.agentrig/skills/${skill}/SKILL.md`);
...
it("pins actual review flow and exact PR-head preparation", async () => {
  const review = instructionSource(".agentrig/skills/review/SKILL.md");
  ...
  const topic = instructionSource(".agentrig/skills/topic/SKILL.md");
...
it("gates every declared reviewer tree on conductor preparation", () => {
 const topic = instructionSource(".agentrig/skills/topic/SKILL.md");
 const prepare = topic.split("- **Prepare.**")[1]?.split("- **Independent conductor checks")[0]?.replace(/\s+/g, " ") ?? "";
 for (const required of ["exit code zero in each reviewer tree before launching any reviewer job", "If any preparation command fails, halt before any launch", "Empty steps run no commands, including bootstrap and preflight", "does not satisfy any reviewer tree", "Reviewers do not run the declared suite, bootstrap or preflight"]) expect(prepare).toContain(required);
});
