...
it.each([null, ...["", "packages/cli/", "packages/core/", "packages/memory/", "packages/supervisor/"].flatMap(base => ["raw/sessions", "wiki"].map(store => `${base}.agentrig/${store}`))])(
  ...
  (base) => {
    ...
    try {
      ...
      if (store !== null) expect(output).toContain("test suite must leave project session stores and wiki untouched");
