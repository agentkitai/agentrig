...
it("actual CLI previews, confirms exact digest, writes parser-compatible skill without applying wiki, and rejects unsafe flags/digests", async () => {
  ...
  try {
    await buildProgram({ config: { env: cliEnv() } }).exitOverride().parseAsync([...args, "--structural-only"], { from: "user" });
    ...
    await buildProgram({ config: { env: cliEnv() } }).exitOverride().parseAsync([...args, "--apply", preview.skillEmission!.digest, "--dream-limits", '{"maxCalls":3}'], { from: "user" });
    ...
    for (const extra of [["--review"], ["--auto"], ["--structural-only"]]) {
      await buildProgram({ config: { env: cliEnv() } }).exitOverride().parseAsync([...args, "--apply", preview.skillEmission!.digest, ...extra], { from: "user" });
      expect(process.exitCode).toBe(1);
    }
    expect(vi.mocked(runDream)).toHaveBeenCalledTimes(2);
    await buildProgram({ config: { env: cliEnv() } }).exitOverride().parseAsync([...args, "--apply", "0".repeat(64), "--dream-limits", '{"maxCalls":3}'], { from: "user" });
    expect(process.exitCode).toBe(1);
