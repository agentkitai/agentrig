...
const fixture=vi.hoisted(()=>({exercise:undefined as undefined|((c:TuiController,input:{send(...chunks:string[]):void},writes:string[])=>Promise<void>),requests:[] as ModelRequest[],clipboardCalls:0}));
...
vi.mock("ink",async importOriginal=>{
  ...
  return {...original,render:(element:Parameters<typeof original.render>[0]&{props:{controller:TuiController}})=>{
    ...
    const done=(async()=>{await vi.waitFor(()=>expect(writes.join("")).toContain("type a task"));await fixture.exercise!(element.props.controller,input,writes);})().finally(()=>ink.unmount());return {unmount:()=>ink.unmount(),waitUntilExit:()=>done};
  }};
});
...
const roots:string[]=[];
afterEach(async()=>{vi.restoreAllMocks();vi.unstubAllEnvs();fixture.requests=[];fixture.clipboardCalls=0;for(const root of roots.splice(0))await rm(root,{recursive:true,force:true});});
it.each(["ordinary","protocol"])("actual startup %s completion and file/image input preserve privacy and literal provenance",async mode=>{
  ...
  fixture.exercise=async(c,input,writes)=>{
    ...
    send(`inspect @${join(root,"sam")}`);send("\t");await vi.waitFor(()=>expect(c.snapshot().pending?.req.tool).toBe("input_directory"));send("y");await vi.waitFor(()=>expect(writes.join("")).toContain("sample.txt"));
    expect(c.permissionGrants.list()).toEqual([]);
    expect(fixture.requests).toHaveLength(0);send("\r");await vi.waitFor(()=>expect(c.snapshot().pending?.req.tool).toBe("input_file"));send("y");await vi.waitFor(()=>{expect(fixture.requests).toHaveLength(1);expect(c.snapshot().status).toBe("idle");},{timeout:5000});
    ...
    send(`@${join(root,"image.png")}`);send("\r");await vi.waitFor(()=>expect(c.snapshot().pending?.req.tool).toBe("input_file"));send("y");await vi.waitFor(()=>{expect(fixture.requests).toHaveLength(2);expect(c.snapshot().status).toBe("idle");},{timeout:5000});
