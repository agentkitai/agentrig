...
export function renderEvent(e: HarnessEvent): string {
  ...
  switch (e.type) {
    ...
    case "subagent.spawn": return `${p} ${e.id} ${JSON.stringify(e.task)}${e.builderProvider === undefined ? "" : ` builderProvider=${e.builderProvider}`}${e.role === undefined ? "" : ` role=${e.role.name} maxTurns=${e.role.maxTurns} tools=${e.role.tools.join(",")}`}`;
