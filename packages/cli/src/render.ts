...
export function renderEvent(e: HarnessEvent): string {
  ...
  switch (e.type) {
    ...
    case "subagent.spawn": return `${p} ${e.id} ${JSON.stringify(e.task)}${e.role === undefined ? "" : ` role=${e.role.name} maxTurns=${e.role.maxTurns} tools=${e.role.tools.join(",")}${e.role.provider === undefined ? "" : ` provider=${e.role.provider}`}`}`;
