...
export async function trainChildEnvironment(cwd: string, profile?: string, builderProvider?: string): Promise<NodeJS.ProcessEnv> {
  ...
  const resolved = resolveConfig({ defaults: {}, cli: {}, env: {}, ...(user === undefined ? {} : { user }), ...(config === undefined ? {} : { project: config }), ...(selectedProfile === undefined || selectedProfile === "recommended" ? {} : { profile: selectedProfile }) });
  if (builderProvider !== undefined && builderProvider !== "default" && !Object.hasOwn(resolved.providers ?? {}, builderProvider)) {
    throw new Error(`BUILDER_PROVIDER_UNKNOWN: declare builderProvider ${JSON.stringify(builderProvider)} in the active profile's providers or use default`);
  }
