...
export function rollupTrainUsage(records: SpendRecord[], rows: Array<{ row: string; sessions: string[] }>, spawns: Array<{ parent: string; child: string }>, providers: ReadonlyMap<string, string> = new Map()): RowUsage[] {
  ...
  return rows.map(row => {
    ...
    const totals = empty();
    const grouped = [...sessions].sort(([a], [b]) => a.localeCompare(b)).map(([session, models]) => ({ session, builderProvider: providers.get(session) ?? null, totals: empty(), models: [...models.values()].sort((a, b) => `${a.provider}/${a.model}`.localeCompare(`${b.provider}/${b.model}`)) }));
