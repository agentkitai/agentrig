/**
 * Runs one session in a bare Node process: a tool that never returns and ignores its signal, an
 * abort at 30ms, then `await session.done`. Vitest keeps the event loop alive, so only a separate
...
setTimeout(() => session.control.abort(), 30);
