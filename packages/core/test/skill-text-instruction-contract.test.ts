...
function bypassesLoader(text: string): boolean {
  ...
  const roots = new Set<ts.Symbol | undefined>();
  function pathPart(node: ts.Node, part: RegExp, identifiers?: Set<ts.Symbol | undefined>): boolean {
    if (ts.isIdentifier(node) && identifiers?.has(symbol(node))) return true;
  ...
  while (changed) {
    ...
    for (const node of nodes) {
      const target = ts.isVariableDeclaration(node) ? node.name
      ...
      if (target && value && ts.isIdentifier(target)) {
