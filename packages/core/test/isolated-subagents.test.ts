...
async function fixture(count = 2, options: { parent?: Partial<AgentConfig>; child?: Partial<AgentConfig>; mutateTool?: (tool: AnyTool) => AnyTool; noGit?: boolean; prepare?: (repo: string) => Promise<void>; runThrows?: boolean; pool?: { children: number; tokens: number; usd: number; live: number } } = {}) {
  ...
  const store = new SessionStore({ root: join(repo, "runtime-logs") });
  ...
  const tool = subagentTool({ isolation: "worktree", ...(options.pool ? { ancestorPools: [options.pool], childBudget: { maxTokens: 4 } } : {}), createAgent: options.runThrows ? () => ({ run() { throw new Error("injected synchronous child.run failure"); } }) : createAgent,
    childConfig: () => {
      const index = built++;
      const probe: AnyTool = { name: "probe", description: "local write fixture", inputSchema: z.object({}), permission: "write", paths: () => ["shared.txt"], effects: "workspace",
      ...
      return { ...base, tools: [probe], provider: provider([[call("write", "probe", {}), stop]]), ...options.child };
    } });
  const config = { ...base, tools: [options.mutateTool?.(tool) ?? tool], turnStrategy: parallel(),
    provider: provider([[...Array.from({ length: count }, (_, n) => call(`child-${n}`, "subagent", { task: `local child ${n}` })), stop]]), ...options.parent };
  ...
  return { root, repo, store, session, finished, entered, gates, cwds, seen, built: () => built };
}
...
it("runs two writer children concurrently in distinct raw-baseline worktrees and returns checked parent diffs", async () => {
  ...
  try {
    ...
    const apply: AnyTool = { name: "apply_candidate", description: "trusted local fixture", inputSchema: z.object({}), permission: "write", paths: () => ["shared.txt", "child-0.txt"],
...
it.each(["symlink", "oversize"])("refuses unsupported %s baseline before spawning a child", async kind => {
  const f = await fixture(1, { prepare: async repo => {
    if (kind === "symlink") { await mkdir(join(repo, "target-dir")); await symlink(join(repo, "target-dir"), join(repo, "alias-link"), process.platform === "win32" ? "junction" : "dir"); }
...
it.each(["non-git", "retention", "child.run"])("fails closed for %s without writing parent files", async kind => {
  const pool = { children: 0, tokens: 0, usd: 0, live: 0 };
  const f = await fixture(1, { pool, noGit: kind === "non-git", runThrows: kind === "child.run",
    ...(kind === "retention" ? { prepare: async (repo: string) => { const root = join(repo, ".git", "agentrig-isolated"); await mkdir(root); for (let n = 0; n < 8; n++) await mkdir(join(root, `retained-${n}`)); } } : {}) });
  const { events } = await f.finished; expect(results(events)[0]!.ok).toBe(false);
  expect(results(events)[0]!.display).toContain(kind === "non-git" ? "not a git repository" : kind === "retention" ? "retention limit" : "injected synchronous child.run failure");
  expect(await readFile(join(f.repo, "shared.txt"), "utf8")).toBe("dirty baseline\n");
