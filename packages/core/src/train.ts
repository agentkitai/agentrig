...
export const TrainRowSchema = z.object({
  task: text,
  authorization: text,
  builderProvider: z.string().min(1).max(128).regex(/^[a-z][a-z0-9-]*$/u).optional(),
...
async function validateQueuedRow(path: string, options: Pick<TrainOptions, "childEnvironment" | "testTimeout">): Promise<void> {
  ...
  if (row.builderProvider !== undefined && options.childEnvironment === undefined) throw new Error("builderProvider requires a profile-aware childEnvironment validator");
