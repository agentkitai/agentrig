...
export interface TrainOptions {
  ...
  sleep?: () => Promise<void>;
  /** Resolve an isolated row environment and preflight reviewer slots before dispatch. */
    ...
    while (true) {
      ...
      try {
        ...
        const exec = async (executable: string, argv: string[], retry = false, testTimeout?: number): Promise<string> => {
          ...
          let result = await command({ executable, argv, cwd: env.checkout, log, env: childEnv });
          if (result.code !== 0 && retry && executable === "pnpm" && argv.length === 1 && argv[0] === "test") {
            ...
            if (files.length > 0) {
              ...
              for (const file of files) {
                ...
                const rerun = await command({ executable, argv: isolated, cwd: env.checkout, log, env: childEnv });
          ...
          if (result.code !== 0 && retry && infrastructure(result)) {
            ...
            result = await command({ executable, argv, cwd: env.checkout, log, env: childEnv });
          }
        ...
        const childProfile = env.profile ?? childEnv?.AGENTRIG_CHILD_PROFILE;
        if (childProfile !== undefined) argv.push("--profile", childProfile);
        if (row.resume !== undefined) argv.push("--resume", row.resume.session);
        argv.push(prompt);
        let sessionWrites = Promise.resolve();
        const result = await command({ executable: process.execPath, argv: options.cli === undefined ? argv : [options.cli, ...argv], cwd: env.checkout, log, env: childEnv, resultPath,
          onSession: id => {
        ...
        if (!pinnedResume) {
          ...
          const ancestry = await command({ executable: "git", argv, cwd: env.checkout, log, env: childEnv });
