...
export const EventPayload = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("turn.aborted"), n: z.number().int().positive(), reason: z.string() }),
  ...
  z.object({
    ...
    turn: z.number().int().positive(),
    ref: z.string().regex(/^refs\/agentrig\/(?:worktrees\/[a-f0-9]{64}\/)?[A-Za-z0-9_-]{1,128}\/[1-9][0-9]*$/),
  ...
  z.object({
    ...
    ref: z.string().regex(/^refs\/agentrig\/(?:worktrees\/[a-f0-9]{64}\/)?[A-Za-z0-9_-]{1,128}\/sealed\/[1-9][0-9]*$/),
    commit: z.string().regex(/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/),
    tree: z.string().regex(/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/),
  ...
  z.object({ type: z.literal("subagent.spawn"), id: z.string(), task: z.string(),
    /** Effective named entry at child creation; absent on historical/unknown hosts. Not a role label. */
    builderProvider: ProviderSelectionInfoSchema.shape.entry.optional(),
