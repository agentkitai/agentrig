...
export async function main(args) {
  ...
  if (!(args.length === 5 || (args.length === 7 && profileFlag === "--profile" && profile)) || resolve(prefix) !== prefix) throw new UsageError(USAGE);
  const { profileChildEnv, reviewerHome } = await import("../packages/cli/dist/child-env.js");
  const { parseConfigText, resolveConfig } = await import("../packages/cli/dist/config.js");
  ...
  resolveConfig({ defaults: {}, project: config, ...(profile === undefined ? {} : { profile }) });
  const childEnv = { ...process.env, ...profileChildEnv(undefined, config, profile) };
