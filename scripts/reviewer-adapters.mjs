...
export async function main(args) {
  args = [...args];
  let profile = process.env.AGENTRIG_CHILD_PROFILE;
  const profileIndex = args.indexOf("--profile");
  if (profileIndex !== -1) {
    profile = args[profileIndex + 1];
    if (!profile || profile.startsWith("--")) throw new UsageError(USAGE);
  ...
  const { parseConfigText, assertNoProjectChildEnv } = await import("../packages/cli/dist/config.js");
  ...
  const { resolveChildEnvironment } = await import("../packages/cli/dist/profile-child-env.js");
  const { reviewerHome } = await import("../packages/cli/dist/child-env.js");
  const env = await resolveChildEnvironment(cwd, profile || undefined, process.env, undefined, config);
  const home = reviewerHome(binding.adapter, env);
  ...
  const gitHead = spawnSync("git", ["-C", cwd, "rev-parse", "HEAD"], { encoding: "utf8", env });
  ...
  writeFileSync(`${prefix}.provenance.json`, JSON.stringify({ verdict, reviewedHead, slot, adapter: binding.adapter, model: result.model, assertedModel: verdict.assertedModel, transportModel, home, modelSource: result.modelSource, launch, cwd, promptPath, started, finished: new Date().toISOString(), exit: 0 }, null, 2) + "\n");
}
