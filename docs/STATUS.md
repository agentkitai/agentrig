...
Labels (including empty labels, #577) no longer route into a second dispatch gate.
The direct factory's legacy tool-envelope entry remains for compatibility tests;
it is not registered at pre_tool. No guard predicate or review-resolution gate is removed.
Superseded manual dispatch/spawn matching, pre-edit comparisons, redundant source
integrity checks and manifest copying are removed from effective skills/policy.
...
- R19a: #549 spawn hooks/log queries; #559 provider-bound roles; #561 provider spawn
  provenance correction; #562 includes/assets/flags.
- R19b: #563 CLI contributions/public API; #564 strict namespaced pack config and legacy bridge.
- R19c: #565 scripts/skeleton; #566 skills/policy/includes; #567 source of truth and pack CI lane.
...
Post-R19 product direction remains deferred to the human. LOW observations and wording
followups are advisories under R19 gate-growth rules, not automatic residual rows.
...
their sources even when those URLs appear historically. Regression coverage checks
...
adds strict `CliPack.configSchema` registration and `packs.<name>` file/process
validation before handler dispatch. `packs.ship.reviewers` and `packs.ship.checks`
bridge to existing consumers; top-level keys still work and warn with migration
paths. Existing files need no rewrite; project trust and child environment authority
are unchanged. PLAN §5.1 defines namespace/profile precedence and public readers.
...
Issue #557 builder implementation (review pending): dispatch detects receipt intent anywhere, denies malformed inline/example receipts with standalone formatting guidance, and validates matching standalone round/read-back fields. Repairs above three require a live PR-body human-amendment record bound to that exact round; absent or unrelated amendments fail closed. No ROADMAP completion marker advanced; conductor review and landing remain pending.
...
PR #555 repair round 2 addresses only D1: OLD extraction now uses the same operative lines as repair receipts, headings and sources, excluding blockquoted/fenced historical OLD values while preserving denial for mismatched operative values (including duplicate conflicting receipts). Fail-first historical-receipt regressions and the raw-task-OLD mutant cover the repair; prior C1/C2/X1/X2 coverage remains intact. Independent conductor review/landing pending.
...
## Pending review — issues #552 and #553, bounded FIFO writer wait

The real-FIFO regression now separates an intentionally slow child startup from a two-second
post-start writer-wait deadline under an explicit 15-second test budget. It still requires the
regular-file diagnostic, so bounded-delay, blocking-open, and nonblocking wrong-reason mutants all
fail while the slow correct startup remains a positive control. Repair round: 0/3; independent
conductor review and exact-head CI remain pending. Residuals: none. This standalone test-only issue
has no roadmap completion marker; LOW observations and wording follow-ups are advisories.
...
program construction. Intentional profile tests remain explicit. Repair round 1 applies
that boundary to the four missed evidence, promotion, procedure-candidate, and skill-emission
fixtures and strengthens the regression with a guaranteed-unavailable inherited profile;
C1/X1 are implemented pending independent delta review. Regression and named mutation
proofs accompany the PR; no roadmap completion marker is set.
...
spawn attribution uses append-only ledger and session records, and ChatGPT-login
usage remains explicitly unpriced. `providers.<name>.maxConcurrent` queues shared
entry streams across processes with visible `model.wait` events. Targeted tests
...
Finding indexing uses explicit severity delimiters and preserves indented ATX bytes;
large review posts prefer complete lines so boundary findings remain indexable.
TESTING describes declared slots/checks including zero, Markdown-only LF attributes,
...
names event targets only. Topic routes clean main advances through shipping policy
§1 CI-staleness alongside §3 conflict/material-delta handling; historical MAIN and
the no-redundant-initial-pair rule remain intact. The instruction expectation failed
first on the prior topic text (7 failures / 124 passes), then passed after the fix.
The bounded heading-scanner vocabulary and exact allowlist are documented next to
the scanner without changing its behavior. H5 attribution has its own section, and
the PR #350 repair receipt is nested under its own maintenance row, not Open questions.
Full trio and mutation receipts are recorded in the PR; independent review and
exact-head hosted CI remain conductor-owned. Repair round: 0/3.

## H5b/H5c marker attribution — issue #318 (placement follow-up #336)
...
checkpoint-undo and subagent-worktree Git children from host trace2 event targets, not
`trace2.normalTarget` or `trace2.perfTarget`. Caller-built environments omitting the setting are not guaranteed isolated. A real Checkpointer regression
uses a temporary HOME trace2 event target with a positive control, alongside the retained environment
...
### Issue #502 — declared-check budgets (PR #512)
...
The evalset mechanics setup also has an explicit 30000ms per-hook budget, since test
budgets do not cover hooks. #501's exact-command retry guard is unchanged.
Independent initial reviews passed; repair round 1 addressed the hook-budget finding
