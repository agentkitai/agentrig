...
| Claude 8b | One `git update-ref --stdin` transaction creates the seal ref and compare-deletes old numeric refs. No process per ref. Global `--no-deref` for every transaction command, numeric-only filtering, last-two retention and OID comparison remain. |
