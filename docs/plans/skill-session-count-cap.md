...
reject more than128 distinct publication session references, after deduplication.
Keep the8192-character metadata cap, file/header byte caps, deterministic sorting,
...
the ordinary evidence loader already defaults to128 validated sessions. The core
skill parser independently caps the metadata array at128. The pure serializer is
...
of its publicationSources exercise128 distinct references (duplicated input accepted,
sorted/deduplicated output accepted by the actual core parser) and129 short references
that fit below the character cap. The129 control failed before the guard; no file is
