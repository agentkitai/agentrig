// Deterministic E1 reference traces. Measures harness prompts/turns, not model skill.
import { mkdtemp, readFile, writeFile, realpath, mkdir } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join, resolve } from 'node:path';
import { execFileSync } from 'node:child_process';
import { fileURLToPath } from 'node:url';
import { createAgent, builtinTools, RulePolicy, defaultRules, SessionStore, PermissionGrantRegistry } from '../../packages/core/dist/index.js';
...
for (const [id, task] of Object.entries(tasks)) {
 ...
 // Frozen supplemental extension: literal, foreground, task-local inspection repeated twice.
 calls.push({name:'bash',input:{command:'git status --short'}}, {name:'bash',input:{command:'git status --short'}});
 let index=0, asks=0;
 const grants = new PermissionGrantRegistry();
 const controller = new TuiController({cwd, build:()=>{throw new Error('benchmark never submits TUI tasks');}, permissionGrants:grants});
 const provider={id:'fake',model:'r17-e1-reference',capabilities:{tools:true,parallelTools:false,caching:false,contextWindow:100000},async *stream(){const call=calls[index++]; if(call) {yield {type:'tool_use',id:String(index),...call}; yield {type:'stop',reason:'tool_use'};} else {yield {type:'text_delta',text:'Reference changes complete.'}; yield {type:'stop',reason:'end_turn'};}}};
 const store=new SessionStore({root:join(temp,'sessions',id)});
 const agent=createAgent({provider,tools:builtinTools(),permissions:new RulePolicy(defaultRules),permissionGrants:grants,onAsk:async(req,context,signal)=>{
  asks++;
  const answer=controller.ask(req,context,signal);
  // Same policy before/after: accept an offered default scoped grant via its
  // preview/explicit confirmation, otherwise allow once. Never select allow-all.
  controller.startDefaultPermissionScope?.();
  if(controller.snapshot().pending?.scope?.preview) controller.confirmPermissionScope();
  else controller.answerPermission('allow');
  return answer;
 },systemPrompt:'Execute the E1 reference trace.',store,trustedProjectRoot:cwd,repoMap:false});
 const session=agent.run(task.prompt,{cwd}); const events=[]; for await (const event of session.events) events.push(event); await session.done;
 const checked=await check(prep.receiptPath);
 const row={task:id,permissionPrompts:asks,turnsToDone:events.filter(e=>e.type==='turn.start').length,sessionEnd:events.at(-1)?.type,reason:events.at(-1)?.reason,sessionId:session.id,grants:grants.list(),toolErrors:events.filter(e=>e.type==='tool.end'&&e.result?.isError),check:checked};
 results.push(row); console.log(JSON.stringify(row));
}
await writeFile(join(temp,'results.json'),JSON.stringify(results,null,2)); console.log('ARTIFACTS '+temp);
if(results.some(r=>r.reason!=='done'||!['PASS','BLOCKED'].includes(r.check.outcome)||r.check.behavior!=='PASS')) process.exitCode=1;
