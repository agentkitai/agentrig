...
| User-defined agent roles with tool allowlists | Claude Code agent files, Goose recipes | Trusted local roles intersect tool/turn limits and live grants recursively, including actual dispatch | R15h (done, PR #196) |
...
| 16 | R16a (in progress) → R16b → R16c → R16d → R16e → R16f → R16g → R16h | TUI polish after R15's first group: R16b uses R12b effect lines, R16e waits for R15a, R16f uses R15i when present. R16a/c/d are independent and may run in parallel. |
