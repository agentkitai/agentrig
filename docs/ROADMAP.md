...
and free heuristics stay on, LLM review stays opt-in. The invalid pilot remains separate.
[Results, interruption and limitations](R17f-RESULTS.md); [contract](plans/R17f.md).
...
R16g is done in PR#210 on
main98eb403; all-four post-main CI34093472579 / structure34093472588 passed.
[R16g receipt](https://github.com/agentkitai/agentrig/pull/210#issuecomment-5566491322).
R16h integrates that green main; implementation/review/local gates are complete.
Its exact-head and post-main delivery receipts belong on the final implementation
PR. No committed implementation remains afterward; optional polish stays at END.
...
(CI34089870734 / structure34089870726), without changing its configuration.
#205 is done on main
...
`18df259`, all-four CI34084903634 / structure34084903714 green.
...
CI `34078485717` and structure `34078485698`. Original failure history: #199 merged as
339f795, but its post-main Windows check failed an existing five-second runtime
fixture. That failed run remains failed; the subsequent repaired-main gate restores
delivery. [Repair contract and original failures](plans/runtime-fixture-ci-bound.md).
...
|---|---|---|
| Done — PR #208 | Existing new/fork/switch grant-transition prompt readiness | Repaired main18df259 passed CI34084903634 / structure34084903714; original failure retained |
...
| Reasoning/thinking blocks preserved across turns | Anthropic interleaved thinking, OpenAI reasoning items | Persisted replay, safe display/export and compaction implemented; material review fix verified; all four exact-head/post-main checks green | R15c *(done, PR #193; [contract](plans/R15c.md))* |
...
| R8b *(done, PR #187; [contract](plans/R8b.md))* | `agentrig mcp-serve`: an MCP server (reusing the M7c stdio JSON-RPC plumbing in reverse) exposing `run_task`, `list_sessions`, `read_session`, `memory_search`; permission posture is the *configured* one — serving never implies yolo | cli |
| R8c *(done, PR #192; [contract](plans/R8c.md))* | OTEL sink (carried follow-up): an optional event-stream subscriber mapping `HarnessEvent`s to OTLP spans (session→trace, turn→span, tool→child span), behind `--otel-endpoint`; no dependency added when unused | core (subscriber) + cli |
| R8d *(done, PR #197; [contract](plans/R8d.md))* | Reference web client: one static page on `127.0.0.1`, speaking R8a over a WebSocket bridge to the same controller as the TUI. Exact bearer/Origin/Host validation, bounded transport and actual Chromium controls; all four post-main checks green. Refuse non-loopback binding | cli |
...
| R9b *(done, PR #180; [contract](plans/R9b.md))* | `agentrig eval <session...> --against <profile>` convenience command over E's isolated task fixtures and independent outcome checks. The M6 rubric Grader supplies advisory analysis; print per-task and aggregate results with usage. Add `eval.result` with this interface | supervisor + cli |
| R9c *(done, PR #183; all four post-main checks green; [contract](plans/R9c.md))* | Nightly structure-regression job using the E1 eval-set definition and E2 reports, driven by a scripted provider. This validates mechanics, not task success or model quality; preserve E3's separate live evaluation lane | docs + .github |
...
| R16g *(done, PR #210; [contract](plans/R16g.md))* | Owned idle `/compact` creates a verified maintenance fork and prints a transcript-only estimate delta; `/clear` aliases `/new`; `/doctor` is model-free and `/diff` captures bounded tracked changes against HEAD or a validated checkpoint. Selected provider is sampled once with existing ledger/native guards. | core + cli |
| R16h (done implementation, PR #211; [contract](plans/R16h.md)) | Fixed light/dark palettes, strict permission/history/abort keybindings, and NO_COLOR precedence; no runtime editor or arbitrary ANSI settings. | cli |
...
| Row | Deliverable | Package |
|---|---|---|
| R17a *(gate, not a PR)* | **Dogfood mandate**: from this band on, work on AgentRig runs through AgentRig. The goal loop runs `agentrig` (TUI, `run --ci`, or ACP) as the driver for every R17 row, and each friction it meets is filed as an issue labelled `feel` before the next child starts. STATUS records, per row, which harness built it; a row built elsewhere names the blocking issue. This restores the PLAN §6 exit criterion as a delivery gate. It has no PR of its own: its artifacts (the `feel` label, the STATUS builder rule, and a baseline `docs/plans/R17-baseline.md` with today's cold start, first-token latency, prompts and turns per E1 task and `--help` option count on the fake provider) are the FIRST commit of R17b, before any default changes | docs + .agentrig |
...
R17f has collected all 96 corrected outcomes using the resolved R17b profile, R17d's real
permission/grant policy and R17e's visible events. Frozen E3 tasks/corpus and independent checks
are retained. Two raw segments preserve the incomplete-call stop and authorized unattempted-slot
...
| R18b *(done — PR #481)* | **Conductor resume in place**: a halted/crashed conductor continues its own session from persisted PR ledger/receipts. `agentrig run --resume <session>` for headless ship/topic restores the plan, repair-round counter and child inventory; ship defines resume at each phase. **Resume residuals complete — PR #496 (#482–#486).** | core + cli + skills |
| R18c *(done — PR #488; scoped train residuals #479(b), #493, #495, #497, #498 done — PR #501; declared-budget residuals #479(a)/(c), #502 complete — PR #512; builder-provider routing #517 complete — PR #526)* | **Train as a command**: `agentrig train <dir>` owns queue-directory row files with a zod schema for task, authorization quote, scope and environment; validates checkout between rows, runs headless, verifies landed PR and green post-merge CI, moves rows to done/halted with machine-readable halt reasons, and prints status after each row. Replaces `drain.sh`/`train.sh` | cli + core |
| R18d *(done — PR #489)* | **Structured verdicts and base-pinned review tooling**: a zod verdict records reviewed head, asserted model, findings with severity, file:line and verbatim heading, and verdict. Posting helper and land consume it instead of regex prose; reviews use base-branch skills/adapters, never PR copies. **Review-tooling residuals complete — PR #499 (#490–#492).** | cli + scripts + skills |
| R18e *(done — PR #494)* | **Per-row cost and concurrency**: roll up row/child usage into a ledger priced at configured rates and expose queue status; a per-provider-entry concurrency limit queues work instead of overloading a shared login | core + cli |
...
| R19d (M4) | **Done — #568 + #569.** Move the train out of core into its own package with pluggable row stages; the ship pack supplies prompt, receipt and landing verification; drop core exports. | train + ship pack + core |
...
8. **No external workflow engine** (Temporal/DBOS/Restate, the second pass's suggestion for
   ...
   R15l permits ordinary trusted user-authored SDK composition over R10; it does not
   introduce a workflow loader, DAG scheduler, automatic merge or new durability guarantee.
...
Backlog delivery and R16g PR#210 are complete on all-four-green main98eb403.
R16h is implemented and integrated, with local gates and one independent review
complete; hosted delivery gates are the final step. No new committed milestone
is introduced by optional polish at END. Independent preparation may run in
parallel; do not freeze work merely to serialize merges.
...
passed all four checks. Feature integration and serial merges resume.
...
|---|---|---|
| Repair (done, PR #191) | Child abort-hook regression gate | R15a #188 post-main macOS CI `34054434385` failed an existing real-I/O timing assumption. Controlled persistence reproduces it; deterministic lifecycle phases preserve production deadlines and all orphan/hook assertions. [Contract](plans/child-abort-hook-gate.md). Repaired main `3174475` passed all four checks: CI `34056045886`, structure `34056045804`; original failed run remains failed. |
...
| Repair (done, PR #145) | Windows memory atomic replacement (merged, PR #145) | CI 34016959860 exposed EPERM replacing the wiki index during real concurrent ingest. Separate bounded, cancellation-aware same-temp retry repair; preserve locks, old-target safety and all Windows tests. Main `3c857d0` passed all three platforms in post-merge [CI 34017665328](https://github.com/agentkitai/agentrig/actions/runs/34017665328). See [contract](plans/windows-memory-replace.md). |
...
| 11 | R8a (done, PR #184) → R8b (done, PR #187) → R8c (done, PR #192) → R8d (done, PR #197) | Reusable control transport on the editor standard, MCP serving, telemetry, then an authenticated local web client. |
...
R15e started independently after green R9c while R15a's ACP dependency was pending.
Its existing M6/permission foundations are delivered; this reordering does not
partially complete R15a, R15c or R15f and adds no new milestone hierarchy.
...
## Follow-ups / nice to haves — R17g closeout

The pre-sweep queue has been dispositioned across four package batches. Every
fragment is implemented, already delivered, or explicitly declined with a concrete
reason; declined features are not falsely marked implemented. No nested follow-up
milestones were added. The [historical queue](R17g-FOLLOWUPS-ARCHIVE.md) preserves
the original failures, caveats, links and partial-delivery notes.

| Package | Delivery / per-fragment outcomes |
|---|---|
