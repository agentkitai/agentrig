...
| 16 | R16a → R16b (in progress) → R16c → R16d → R16e → R16f → R16g → R16h | TUI polish after R15's first group: R16b uses completed R12b effect lines and existing Static ANSI support, not R16a's parser; a/b/c/d are independent and may run in parallel. R16e waits for R15a; R16f uses R15i accounting. |
