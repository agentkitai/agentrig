...
| R15h *(in progress; [contract](plans/R15h.md))* | Agent role definitions: `.agentrig/agents/<name>.md` (front-matter validated by the R5e schema: `tools`, `model-role`, `delegable`, `max-turns`, body = system prompt). The subagent tool accepts `agent: <name>`; the child's grants are the R12d delegable view intersected with the role's allowlist. Roles never widen the parent's authority | core |
...
| 14 | R15h (in progress) → R15i → R15j → R15k | Roles after R12d delegation; ledger before R7 unattended runs are enabled by default; TUI conveniences last. |
