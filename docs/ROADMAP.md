...
| R15k *(in progress; [contract](plans/R15k.md))* | TUI input: `@path` completion inserting a read-class file reference, and image paste/`@image.png` producing an image `ContentBlock`; both go through the R13b source seam for provenance | cli |
...
R16a #199 and bounded runtime-fixture repair #206 are done: repaired main
`0b65aa18c0f8629ee86186ccb0d7a1570fe51f3c` passed all four checks, CI
`34078485717` and structure `34078485698`. Original failed main remains failed.
...
R15k remains in progress; feature integration and serial merges resume.
...
| 16 | R16a (done, PR #199 + repair #206) → R16b → R16c (done, PR #198) → R16d (done, PR #200) → R16e → R16f → R16g → R16h | TUI polish after R15's first group: R16b uses R12b effect lines, R16e waits for R15a, R16f uses R15i when present. R16a/c/d are independent and may run in parallel. |
