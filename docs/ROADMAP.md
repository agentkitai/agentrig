...
| H3 | Preserve knowledge before ingest coverage planning: include authoritative assistant messages without duplicating streamed deltas, distinguish model conclusions/requests/denials from tool evidence, and account for long tool outputs with bounded ranges or explicit omissions. Preserve canonical-only records in interrupted logs and report nested non-text omissions. | A conclusion present only in an assistant message and decisive evidence after character 500 both reach distillation. Coverage reports name any uninspected ranges; truncation cannot silently count as coverage. Missing/corrupt logs fail before writes; legacy captures migrate to the corrected coverage contract. |
