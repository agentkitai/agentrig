...
| Row | Deliverable | Package |
|---|---|---|
| R18a *(in flight — drain row 172, #467)* | **Turn-boundary recovery and resumable children**: discard a partial failed model stream, record `turn.aborted`, and re-request once within a bounded per-session recovery budget, then fail. Parent `subagent.end` reports error only after child recovery is exhausted | core |
