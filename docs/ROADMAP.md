...
| 16 | R16a → R16b → R16c (done, PR #198) → R16d (in progress) → R16e → R16f → R16g → R16h | TUI polish after R15's first group: R16b uses R12b effect lines, R16e waits for R15a, R16f uses R15i when present. R16a/c/d are independent and may run in parallel. |
