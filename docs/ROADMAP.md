...
| R18b *(done; pending review)* | **Conductor resume in place**: a halted/crashed conductor continues its own session from persisted PR ledger/receipts. `agentrig run --resume <session>` for headless ship/topic restores the plan, repair-round counter and child inventory; ship defines resume at each phase | core + cli + skills |
