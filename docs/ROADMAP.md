...
|---|---|---|---|
| Structured clarifying question from the model (`ask_user`) | Claude Code, Codex, Cline, Goose | free text only; the supervisor talks to the model, the model has no channel back | R15a *(done — PR #188, restored by repair #191; [contract](plans/R15a.md))* |
...
| R8b *(done — [PR #187](https://github.com/agentkitai/agentrig/pull/187); [contract](plans/R8b.md))* | `agentrig mcp-serve`: an MCP server (reusing the M7c stdio JSON-RPC plumbing in reverse) exposing `run_task`, `list_sessions`, `read_session`, `memory_search`; permission posture is the *configured* one — serving never implies yolo | cli |
...
| 11 | R8a (done, PR #184) → R8b (done, PR #187) → R8c → R8d | Reusable control transport on the editor standard, MCP serving, telemetry, then an authenticated local web client. |
