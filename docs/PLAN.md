...
1024 characters, shared by the event and update_plan tool schemas. The first request of a run
with that tool asks for a check per item, without replacing custom prompts or manufacturing fresh
user consent. Tool/session/plan displays retain declarations and mark missing or declared checks
