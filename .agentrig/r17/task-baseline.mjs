...
import { createAgent, builtinTools, RulePolicy, defaultRules, SessionStore } from '../../packages/core/dist/index.js';
...
for (const [id, task] of Object.entries(tasks)) {
 ...
 const agent=createAgent({provider,tools:builtinTools(),permissions:new RulePolicy(defaultRules),onAsk:async()=>{asks++;return 'allow';},systemPrompt:'Execute the E1 reference trace.',store,trustedProjectRoot:cwd,repoMap:false});
 ...
 const row={task:id,permissionPrompts:asks,turnsToDone:events.filter(e=>e.type==='turn.start').length,sessionEnd:events.at(-1)?.type,reason:events.at(-1)?.reason,sessionId:session.id,toolErrors:events.filter(e=>e.type==='tool.end'&&e.result?.isError),check:checked};
