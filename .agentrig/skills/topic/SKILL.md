...
- The human's sentence invoking this skill is the merge authorization for the whole named band.
  Capture that sentence byte-for-byte as `AUTHORIZATION`; do not paraphrase, normalize, or infer it.
  Quote it verbatim in the parent's final report and require it in every landed PR's description
  and squash-merge commit body. Landing remains a human decision: this sentence is the decision,
  made once up front under this skill's review, CI, and stop criteria. Silence, confidence, a review
  verdict, or a prior unrelated merge instruction is never authorization.
- Read the roadmap and expand exactly one named band before spawning work. For example, `R2` expands
  to R2a, R2b, R2c, and R2d in roadmap order. Record the fixed row list. If the band or invocation
  sentence is ambiguous, stop and ask the human before any child or branch is created.
...
1. Confirm `main` contains the preceding row's merge (unless this is the first row) and CI is green
   on that exact merge commit. Never stack PRs and never begin while the prior land check is pending.
2. Spawn a builder subagent with a self-contained task containing the exact roadmap-row contract,
   ...
   authorization verbatim in the PR description. Report the PR, current head SHA and CI, both
   external reviews, and your session id.” The dogfood child stops at its PR and never merges.
3. If the builder dies at its budget or leaves a half-pushed branch, halt. Report its session id for
   `agentrig sessions resume <id>`; never replace it with a fresh builder.
4. Spawn a new reviewer subagent with only: “Review PR #NN on its current head. Follow the review
   skill. Report your session id.” Never pass the builder's report, reasoning, findings, or claimed
   evidence to this reviewer. The PR and repository are its only evidence.
5. Record every child session id as soon as it is returned. If the reviewer dies at budget, cannot
   verify any claim, sees a merge conflict, finds red CI on the actual head, or reports a MEDIUM or
   HIGH finding, halt without rebutting or fixing it yourself.
...
- A clean verdict proceeds to landing. A verdict containing only LOW findings gets exactly one
  ...
  no other changes. Tell it not to rebut or skip a finding, to add fail-first proof where behavior
  changes, run the green trio, push, and report the old/new head SHAs plus its session id.
...
- Otherwise spawn one fresh delta-review subagent. Give it the PR number and old/new head SHAs, ask
  ...
  report. Land only on a clean, fully verified delta verdict. Any finding of any severity, or any
...
- After a clean full review, or a clean one-time delta review, independently confirm CI is green on
  the PR's actual current head SHA. Then spawn a land subagent: “Land PR #NN following the land
  skill. The human authorized this row as part of BAND with the following exact invocation:
  `AUTHORIZATION`. Preserve that quote verbatim in the PR description and squash-merge commit body.”
